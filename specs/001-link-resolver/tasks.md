...
- [ ] T015 [P] [US1] CLI contract test for resolved outcomes (plain, heading, block, same-file) asserting single-line JSON on stdout and exit 0 in `tests/contract/resolve_cli.rs` (quickstart scenarios 1–4)
- [ ] T016 [P] [US1] CLI contract test for `unresolved` (exit 2) and `sub_target_not_found` (exit 3, `target_path` + `reason` present) in `tests/contract/resolve_errors.rs` (quickstart scenarios 5, 6; FR-019)
- [ ] T017 [P] [US1] Integration test over the fixture vault for scenarios 1–6, 10 (alias), 11 (embed), 12 (markdown-style), 13 (attachment), 14 (path-qualified), 16 (explicit `--vault`) in `tests/integration/us1_resolution.rs`
- [ ] T018 [P] [US1] Unit tests for link parsing (wikilink/markdown styles, embed flag, alias split on `|`, folder prefix, `#` heading path, `^` block id, self-reference, whitespace trimming, and the rule that a link cannot target both a heading and a block id) in `tests/unit/link_parse.rs`
