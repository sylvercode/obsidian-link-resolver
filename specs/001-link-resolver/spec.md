...
- Q: Which Obsidian reference rules must the specification explicitly encode beyond the resolution behavior already covered? → A: The specification must explicitly cover OR1-OR14: supported file types, disallowed dotfiles/dotfolders and invalid characters, wikilink vs markdown syntax, alias/display text, embed flag, relative-path semantics, heading/block references, same-file self-links, nested heading traversal, and block-id rules.
...
- **Structured block emplacement**: When a target line belongs to a structured block (list, table, quote, or callout), the structured emplacement MUST include not only the containing headings but also the start and end lines of the enclosing structured block itself. For list items, the list item line must be delimitable within its parent list block and the ancestor headings containing that list, preserving the parent list context as well as the heading stack (OR12.1, OR12.2).
...
- **Display text and alias semantics**: A display label or alias affects only rendering and must never change the target resolution; the underlying target is determined from the file name or path and any heading/block fragment, not from the alias text (OR6.2, OR13).
...
- **FR-002d**: The system MUST support both link styles defined by Obsidian: wikilinks in `[[...]]` form and markdown links in `[text](target)` form, with the same target semantics; it MUST parse optional display text/alias text, trim whitespace around each segment, and treat the embed prefix `!` as presentation-only metadata rather than a semantic change to the resolved target (OR6, OR13, OR14).
...
- **FR-008**: The system MUST, on request, return the structured emplacement of the target: the ordered stack of containing headings (outermost to innermost), the enclosing structured-block boundaries when the target falls inside a list, table, quote, callout, or other structured block, and the begin/end lines of each relevant section. For a list item, the emplacement MUST include the list item line within the parent list block and its containing heading sections.
- **FR-009**: The system MUST compute each heading section's end as the line immediately before the next heading of equal or higher level (or end of file if none follows). When the target belongs to a structured block, the system MUST also delimit the start and end lines of that enclosing block (for tables, callouts, quotes, and lists) and, for list items, the list item line must be represented as part of the parent list block context rather than as a standalone section outside that list hierarchy.
