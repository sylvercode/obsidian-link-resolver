...
- **FR-006**: For a resolved target, the system MUST return the target file path and, when the link points to a heading, block, or structured-block target, the target's full line range as `target_range: { begin, end }`. A structured block target is always represented as a range, even when it occupies only one line, in which case the range is `{begin: X, end: X}`. The `target_line` field is a convenience value equal to `target_range.begin` for heading/block/structured-block targets, but `target_range` is the canonical location contract. The target file path and every candidate path MUST be expressed as a vault-relative path with forward-slash (`/`) separators (never an absolute filesystem path), so results remain portable and byte-for-byte deterministic across machines (FR-016, SC-003).
- **FR-007**: When the link has no heading or block component, the system MUST return the target file with the beginning of the file as the target location. For plain-file and attachment targets, `target_range` is null/absent and `target_line` is null/absent; for a single-line or single-point target, the range is still represented with `begin` and `end` equal to that line when a range-bearing note target is applicable.
