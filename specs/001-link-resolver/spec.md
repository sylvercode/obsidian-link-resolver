...
- **Duplicate headings**: A note contains two headings with the same text; the resolver selects the target consistent with Obsidian's resolution behavior and reports which one was chosen.
- **Nested heading path**: A link like `[[Note#Parent#Child]]` targets the `Child` heading nested under `Parent`.
- **Path-qualified note name**: A link like `[[folder/subfolder/Note#Heading]]` includes a folder path (relative to the vault root) before the note name; the resolver uses that path to disambiguate and target the specific note under that folder.
...
- **FR-005**: The system MUST resolve a note name to a target file consistent with Obsidian's link resolution behavior (including shortest-path / vault-wide matching).
- **FR-005a**: The system MUST support path-qualified note names, where a link includes a folder path (relative to the vault root) before the note name (e.g., `[[folder/subfolder/Note]]`), and MUST resolve to the note located at that path relative to the vault root.
...
- **FR-010**: The system MUST distinguish and report the following outcomes: resolved, unresolved (target note not found), sub-target-not-found (note found but heading/block missing), and ambiguous (multiple candidate targets).
...
- **FR-017**: The system MUST communicate outcome via a distinct, documented exit status so callers can branch on success, unresolved, ambiguous, and error conditions without parsing text.
