...
fn resolves_documented_fixture_scenarios() {
    ...
    assert_eq!(resolved.target_range, None);
    ...
    assert_eq!(
        heading.target_range.as_ref().map(|range| range.begin),
        Some(5)
    );
    ...
    assert_eq!(
        block.target_range.as_ref().map(|range| range.begin),
        Some(9)
    );
    ...
    assert_eq!(
        same_file.target_range.as_ref().map(|range| range.begin),
        Some(1)
    );
    ...
    assert_eq!(
        missing_sub_target.target_path.as_deref(),
        Some("Project Plan.md")
    );
